...
class JobSummaryRequest(BaseModel):
    ...
    job_description: str = Field(
        ..., description="Full job description (can be HTML format)"
    )
...
class JobSummaryResponse(BaseModel):
    ...
    original_length: int = Field(
        ..., description="Original description length in characters"
    )
    summary: str = Field(..., description="Generated summary")
    summary_length: int = Field(..., description="Summary length in words")
    key_points: List[str] = Field(
        ..., description="Key highlights extracted from the job"
    )
