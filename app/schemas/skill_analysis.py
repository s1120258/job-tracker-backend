...
class TechnicalSkill(BaseModel):
    ...
    years_experience: int = Field(0, ge=0, description="Years of experience with this skill")
    evidence: Optional[str] = Field(None, description="Evidence from resume text supporting this skill")
...
class SkillRequirement(BaseModel):
    ...
    importance: Importance = Field(..., description="Importance level of this skill for the job")
...
class SkillGap(BaseModel):
    ...
    current_level: SkillLevel = Field(..., description="Current proficiency level of candidate")
    priority: Priority = Field(..., description="Priority level for addressing this gap")
...
class LearningRecommendation(BaseModel):
    ...
    estimated_learning_time: str = Field(..., description="Estimated time to acquire the skill")
    ...
    immediate_actions: List[str] = Field(default_factory=list, description="Immediate steps to take")
...
class ExperienceGap(BaseModel):
    ...
    candidate_years: int = Field(..., ge=0, description="Candidate's years of experience")
...
class SkillGapAnalysisResponse(BaseModel):
    ...
    overall_match_percentage: float = Field(..., ge=0, le=100, description="Overall match percentage")
    match_summary: str = Field(..., description="Summary of the skill match")
    strengths: List[SkillStrength] = Field(default_factory=list, description="Candidate's skill strengths")
    ...
    experience_gap: Optional[ExperienceGap] = Field(None, description="Experience gap analysis")
    education_match: Optional[EducationMatch] = Field(None, description="Education requirement match")
...
class SkillGapAnalysisRequest(BaseModel):
    ...
    resume_id: Optional[UUID] = Field(None, description="Specific resume ID to use (optional)")
    include_learning_recommendations: bool = Field(True, description="Whether to include learning recommendations")
    include_experience_analysis: bool = Field(True, description="Whether to include experience gap analysis")
    include_education_analysis: bool = Field(True, description="Whether to include education matching")


class SkillExtractionRequest(BaseModel):
    """Schema for skill extraction request."""
    text: str = Field(..., min_length=1, description="Text to extract skills from")
    context: str = Field("resume", description="Context of the text (resume, job_description)")
