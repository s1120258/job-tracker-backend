...
class TestJobSkillsExtraction:
    ...
    def test_extract_job_skills_success(self, mock_extract_skills, mock_get_job, mock_user, mock_job, mock_skills_data):
    ...
    @patch('app.api.routes_jobs.crud_job.get_job')
    @patch('app.api.routes_jobs.skill_extraction_service.extract_skills_from_job')
    def test_extract_job_skills_service_error(self, mock_extract_skills, mock_get_job, mock_user, mock_job):
...
class TestResumeSkillsExtraction:
    ...
    def test_extract_resume_skills_success(self, mock_extract_skills, mock_get_resume, mock_user, mock_resume, mock_resume_skills_data):
    ...
    @patch('app.api.routes_resumes.crud_resume.get_resume_by_user')
    def test_extract_resume_skills_empty_text(self, mock_get_resume, mock_user, mock_resume):
...
class TestSkillGapAnalysis:
    ...
    def test_analyze_skill_gap_success(self, mock_analyze_gap, mock_get_resume, mock_get_job,
                                      mock_user, mock_job, mock_resume, mock_gap_analysis_data):
    ...
    def test_analyze_skill_gap_job_not_found(self, mock_get_job, mock_user):
        ...
        non_existent_job_id = "00000000-0000-0000-0000-000000000000"
        response = client.get(f"{API_V1_PREFIX}/jobs/{non_existent_job_id}/skill-gap-analysis")
    ...
    @patch('app.api.routes_jobs.crud_job.get_job')
    @patch('app.api.routes_jobs.get_resume_by_user')
    def test_analyze_skill_gap_no_resume(self, mock_get_resume, mock_get_job, mock_user, mock_job):
        """Test when user has no resume"""
        mock_get_job.return_value = mock_job
        mock_get_resume.return_value = None
    ...
    @patch('app.api.routes_jobs.crud_job.get_job')
    @patch('app.api.routes_jobs.get_resume_by_user')
    def test_analyze_skill_gap_empty_resume_text(self, mock_get_resume, mock_get_job,
                                                mock_user, mock_job, mock_resume):
        """Test when resume has no extracted text"""
        mock_get_job.return_value = mock_job
        mock_resume.extracted_text = ""
        mock_get_resume.return_value = mock_resume
    ...
    @patch('app.api.routes_jobs.get_resume_by_user')
    @patch('app.api.routes_jobs.skill_extraction_service.analyze_skill_gap')
    def test_analyze_skill_gap_service_error(self, mock_analyze_gap, mock_get_resume, mock_get_job,
                                            mock_user, mock_job, mock_resume):
        """Test skill extraction service error"""
        mock_get_job.return_value = mock_job
        mock_get_resume.return_value = mock_resume
